...
// mv.cc の sell32_spmv_kernel を完全に置き換えてください
...
)
{
    ...
    for (uint32_t b = 0; b < num_blocks; b++) {
        ...
        // --- Width Loop (最重要: ここの回転効率が性能を決める) ---
        // ell_width は通常十分大きいので、パイプライン化を指示
...
{
    // [重要] コンパイラにスケジューリングのためのCPU時間をフルに使うよう指示 (0.0~1.0)
    // これにより、命令の詰め込み密度が限界まで上がります
    AIE_MODULO_SCHEDULING_BUDGET_RATIO(1.0);
    ...
    // コンパイル時定数として認識させる（最適化の助けになる）
    const int block_stride = 64; 

    // outer loopのオーバーヘッドは小さいので、ここではそのまま
    for (uint32_t b = 0; b < num_blocks; b++) {
        ...
        // ループ回数がコンパイル時に不明でも、可能な限り展開させる
        #pragma clang loop unroll_count(4) 
        for (uint32_t k = 0; k < ell_width; k++) {
            
            // 1. ベクトルロード (Load bandwidth saturation)
            // AIEの2つのロードポートを最大活用
            aie::vector<uint16_t, 16> idx0 = aie::load_v<16>(p_idx_curr);
            aie::vector<uint16_t, 16> idx1 = aie::load_v<16>(p_idx_curr + 16);
            ...
            // ポインタ更新: コンパイラがAddress Generation Unit (AGU) の
            // ポストインクリメントなどを使いやすい位置に配置
            p_idx_curr += block_stride;
            p_val_curr += block_stride;

            // 2. Gather (The Bottleneck)
            // ここがスカラ処理。
            // コンパイラはこのループを「ベクトル命令の隙間」に分散配置しようと努力します。
            aie::vector<bfloat16, 16> x_gathered0;
            aie::vector<bfloat16, 16> x_gathered1;

            // [最適化] ループ内での計算を排除し、単純なメモリアクセスのみにする
            // __restrictをつけているため、vec_xの読み込みが他と干渉しないことをコンパイラは知っている
            #pragma unroll
            for (int i = 0; i < 16; i++) {
                x_gathered0[i] = vec_x[idx0[i]];
            }
...
)
{
    ...
    // ポインタのセットアップ
    // データレイアウト: [32 indices (uint16)] [32 values (bf16)]
    const uint16_t *__restrict ptr_idx = reinterpret_cast<const uint16_t*>(data_ptr);
    const bfloat16 *__restrict ptr_val = data_ptr + 32; 

    // ブロックストライド（要素数換算ではなく、ポインタが進む距離として定義）
    // 32 indices + 32 values = 64 elements stride
    const int block_stride = 64; 
    ...
    for (uint32_t b = 0; b < num_blocks; b++) {
        ...
        // Gatherのレイテンシを隠蔽するためにアンロール
        //#pragma clang loop unroll_count(10)
        for (uint32_t k = 0; k < ell_width; k++) {
            
            aie::vector<uint16_t, 32> idx = aie::load_v<32>(p_idx_curr);
            aie::vector<bfloat16, 32> val = aie::load_v<32>(p_val_curr);
...
)
{
    ...
    // コンパイル定数として認識されやすいようにconst指定
    const int block_stride = 64; 

    for (uint32_t b = 0; b < num_blocks; b++) {

        // 32要素アキュムレータ (1024-bit)
        aie::accum<accfloat, 32> acc = aie::zeros<accfloat, 32>();
        ...
        for (uint32_t k = 0; k < ell_width; k++) {
            ...
            // 2. Gather
            // 32回のスカラアクセスが発生する重い処理。
            // 2回アンロールされた状態なら、コンパイラは
            // 「次のループのLoad命令」をこの隙間にねじ込んでくれます。
            aie::vector<bfloat16, 32> x_gathered;
...
template <uint32_t r = 32> 
void sell32_spmv_kernel_coffset(
    uint32_t num_blocks,
    uint32_t ell_width,
    uint32_t c_offset,
    const bfloat16 *__restrict data_ptr, 
...
template <uint32_t r> //floatは4/8/16/32, int16,bfloat16は8/16/32/64まで対応
void sparse_matvec_vectorized_aligned_notc(uint32_t m, 
                                uint32_t k,
                                uint32_t ell_width, 
                                uint32_t row_offset, 
                                const bfloat16 *__restrict a, 
                                    ...
                                    )
{
    ...
    //sparse_matvec_vectorized_aligned_notc<32>(m, k, ell_width, row_offset, a_in, b_in);
...
void sell32_spmv_coffset_vectorized_bf16_bf16(
    uint32_t num_blocks,
    uint32_t ell_width,
    uint32_t c_offset,
    const bfloat16 *data_ptr,
...
{
    sell32_spmv_kernel_coffset<32>(num_blocks, ell_width, c_offset, data_ptr, vec_x, vec_y);
}
