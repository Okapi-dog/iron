...
void sell32_spmv_kernel_16_res(
    uint32_t num_blocks,
    uint32_t ell_width,
...
)
{
    ...
    const bfloat16 *__restrict ptr_curr = data_ptr;

    // --- Block Loop ---
    for (uint32_t b = 0; b < num_blocks; b++) {

        // アキュムレータを16要素(FP32)×2つに分割
        aie::accum<accfloat, 16> acc0 = aie::zeros<accfloat, 16>();
        aie::accum<accfloat, 16> acc1 = aie::zeros<accfloat, 16>();
        ...
        for (uint32_t k = 0; k < ell_width; k++) {
            ...
            // ロード: 32要素一括 (ここはデータレイアウト的に安全)
            aie::vector<uint16_t, 32> idx_vec = aie::load_v<32>(reinterpret_cast<const uint16_t*>(ptr_curr));
            ptr_curr += 32;

            aie::vector<bfloat16, 32> val_vec = aie::load_v<32>(ptr_curr);
            ptr_curr += 32;

            // 分割 (extractはゼロコスト)
            auto idx0 = idx_vec.extract<16>(0);
            auto idx1 = idx_vec.extract<16>(1);
            auto val0 = val_vec.extract<16>(0);
            auto val1 = val_vec.extract<16>(1);
        ...
        // ★修正: ストアも16要素ずつ2回に分けて行う (最も確実な方法)
        // 32要素ベクトルの生成(insert)すら行わず、直接メモリへ書き込みます
        aie::store_v(vec_y, acc0.template to_vector<bfloat16>());
        aie::store_v(vec_y + 16, acc1.template to_vector<bfloat16>());
...
{
    sell32_spmv_kernel_16_res<32>(num_blocks, ell_width, data_ptr, vec_x, vec_y);
}
