...
# --- 1. Float32 を BFloat16 のビットパターン(int16)に変換する関数 ---
def float32_to_bf16_bits_as_int16(arr_float32):
    """
    ...
    # 3. int16型にキャストして返す
    #    (NPU転送関数が int16 を要求しているため)
    return bf16_bits_u32.astype(np.int16)
...
def pack_for_xdna(ell_data, ell_indices):
    ...
    # 1. Indexを int16 にキャスト
    #    (列数が32767を超えるとオーバーフローして負になりますが、bit的には合っているのでuint16解釈ならOK)
    indices_int16 = ell_indices.astype(np.int16)
    
    # 2. Value(float32) を bf16ビットパターン(int16) に変換
    values_bf16_as_int16 = float32_to_bf16_bits_as_int16(ell_data.astype(np.float32))
    
    # 3. インターリーブ（交互配置）の作成
    #    Shapeを (Rows, 2, Width) にして、dim1=0にIndex, dim1=1にValueを置く
    combined = np.empty((rows, 2, width), dtype=np.int16)
    combined[:, 0, :] = indices_int16
    combined[:, 1, :] = values_bf16_as_int16
...
def process_matrix_for_npu(group, name, output_dir="npu_data"):
    ...
    # 配列確保
    ell_data = np.zeros((n_rows, max_nnz), dtype=np.float32)
    ell_indices = np.zeros((n_rows, max_nnz), dtype=np.int32) # 一旦int32で
    ...
    print(f"ELL Shape: {ell_data.shape} (Width: {max_nnz})")
    #ellを確認
    print("\n--- ELL Data ---")
    print(f"Data:\n{ell_data}")
    print(f"Indices:\n{ell_indices}")
    ...
    #NPUフォーマットへ変換
    print("Packing for XDNA NPU (bf16/int16 interleaved)...")
    npu_buffer = pack_for_xdna(ell_data, ell_indices)
    
    #保存 (NPU用バイナリデータとして .npy で保存)
    save_path = os.path.join(save_dir, f"{name}_xdna_int16.npy")
    ...
    print("Row 0 Indices (int16):", npu_buffer[0:max_nnz])
    # bf16部分は整数として表示されるので人間には読みにくいですが、値が入っていればOK
    print("Row 0 Values  (bf16 as int16):", npu_buffer[max_nnz:max_nnz*2]) 
...
def print_npu_matrix(matrix_npy_path,cols):
    ...
    print("Indices (int16):", row0_indices)
    print("Values (bf16 as int16):", row0_values)
