...
| 記号 | 意味 | 主なトレードオフ |
|---|---|---|
| `R` | 一つの MemTile 配下にある物理 core-row 数（現状は 4） | ハードウェア配置 |
...
これによりblockごとにBF16へ丸め直す誤差を避ける。最初の候補は`V=32`,
`C_h={2,4,8,16}`, `B_h=R*C_h={8,16,32,64}`, `B_w={128,256}`とする。
従って32 laneを使うことは`B_h>=128`を意味しない。最初の実装における実用上の最小slice
...
各 weight matrix ごとに、まず `B_w={32,64,128,256,512}` と L1 に入る `C_h` を列挙します。
row ごとの nnz を一度数えれば、各候補について
次を計算できます。
...
まず、転送量・object 数・L1 使用量の三目的で支配される候補を除外し、残る Pareto
frontier だけを実機で測ります。ある候補が、別候補より転送量・object 数・L1 使用量の
すべてで悪いなら、測定する必要はありません。
...
### Phase 2: format と reference packer を先に作る

1. `R=4`、候補 `C_h={2,4,8,16}`、`B_w={128,256}` の row-order-preserving Slice-ELL packer
   を Python で作る。
2. `packed_A`, `slice_blocks`, `manifest.json` を出力する。`slice_blocks[s]=0`、末尾 row、
   padding index=0、複数 Shim column への連続 slice 割当を含める。runtime用のhost referenceは
   xとcolumn-local `slice_blocks`から固定長`packed_config`を生成する。
3. 同じ packed format を読む Python reference を作り、元の sparse matrix reference と
   全行一致する property test を作る。NPU を使わずここまで検証する。
4. matrix ごとに Pareto table を出し、実際に選ばれる `(C_h,B_w)` と容量・padding を manifest
   に記録する。
...
### Phase 5: 実 model layer への統合と autotuning

1. 代表四 shape に対し、matrix ごとの packer / manifest / xclbin artifact を生成する。
2. 候補 `(C_h,B_w)` の Pareto frontier だけを NPU で測り、`Tmodel` の DMA bandwidth、
   object overhead、kernel throughput を当てはめる。
3. 選択された matrix ごとの parameter と packed data を再現可能な command で保存し、
   fixed ELL、SELL-32、Slice-ELL を同じ精度・同じ input vector で比較する。
