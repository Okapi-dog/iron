...
def my_matvec(dev, num_cores, M, K, m, trace_ddr_id=None, trace_size=65536):
    ...
    A_L3L1_fifos = [ObjectFifo(L1_A_ty, name=f"A_L3L1_{i}", depth=1) for i in range(num_cores)]
    A_L3L1_fifos2 = [ObjectFifo(L1_A_ty, name=f"A_L3L1_2_{i}", depth=1) for i in range(num_cores)]
    C_L1L3_fifos = [
        ObjectFifo(L1_C_ty, name=f"C_L1L3_{i}", depth=1) for i in range(num_cores)
    ]

    def core_body(A_L3L1_fifo, A_L3L1_fifo2, C_L1L3_fifo):
        one_idx = index.constant(1)
        m_idx = index.constant(m)
        for _ in range_(0xFFFFFFFF):
            C_L1L3_fifo.acquire(1)
            for i_idx in range_(M // m // num_cores):
                A_L3L1_fifo.acquire(1)
                A_L3L1_fifo2.acquire(1)
                A_L3L1_fifo2.release(1)
    ...
    with rt.sequence(L3_A_ty, L3_B_ty, L3_C_ty) as (A, B, C):
        if trace_ddr_id is not None:
            rt.enable_trace(
                ...
                workers=[workers[0],workers[0],workers[1],workers[1]],
        ...
        for num_core in range(num_cores):
            ...
            rt.fill(
                ...
                A, 
                A_taps[num_core], #全てのコアが同じデータを読み取るように変更
                task_group=tg,
                placement=shim_tile
            )
            rt.fill(
                A_L3L1_fifos2[num_core].prod(), 
                A, 
                A_taps[num_core], #全てのコアが同じデータを読み取るように変更
                task_group=tg,
                placement=shim_tile
